...
def get_subcolumn_mask(cldf, seeds, ngpt: int, overlap_option: int = 1):
    ...
    Returns an int mask (ncol, nlay, ngpt)."""
    ...
    for l in range(1, nlay):
        above = cols[l - 1]
        cloudy_above = above > 1.0 - cldf_e[:, l - 1, 0][:, None]
        cols.append(jnp.where(cloudy_above, above,
                              cldx[:, l, :] * (1.0 - cldf_e[:, l - 1, 0][:, None])))
