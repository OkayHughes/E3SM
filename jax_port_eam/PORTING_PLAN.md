...
| 12 | CLUBB | clubb/ | very large | **slices A+B delivered** (A: grid + saturation + dgtsv tridiag + pdf_closure ADG1; B: pdf_closure_driver — zt+zm double call, trapezoidal rule, clip_rcm, compute_cloud_cover — kernel-golden vs a verbatim extraction validated BITWISE end-to-end against advance_clubb_core); next slice: **C** (advance_xp2_xpyp); see the CLUBB slicing plan below |
...
- **C (NEXT)**: advance_xp2_xpyp (tridiag LHS/RHS assembly,
  turbulent_adv_pdf, diffusion, mean_adv, clip_explicit,
  pos_definite_variances, l_iter_xp2_xpyp=T two-pass iteration) +
  clip_covars_denom.  advance_xp2_xpyp is PUBLIC — golden it directly
  with a drv_ wrapper (no transcription needed); its inputs
  (tau_zm/Kh_zt/wp3_on_wp2/Lscale/wp2_splat/...) can be captured from
  drv_advance_clubb_core-advanced states or synthesized; under
  placement=2 the sub-advances run BEFORE the pdf call, so a chained
  bitwise check like slice B's needs the pre-advance module state —
  simplest is direct kernel goldens plus the existing whole-advance
  harness for Tier-1.5 later (slice H).
- **D**: Lscale/tau infrastructure: compute_mixing_length (+ the
  l_avg_Lscale perturbed calls), calc_brunt_vaisala_freq_sqd,
