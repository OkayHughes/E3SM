...
| 12 | CLUBB | clubb/ | very large | **slices A+B+C+D delivered** (A: grid + saturation + dgtsv tridiag + pdf_closure ADG1; B: pdf_closure_driver — zt+zm double call, trapezoidal rule, clip_rcm, compute_cloud_cover — kernel-golden vs a verbatim extraction validated BITWISE end-to-end against advance_clubb_core; C: advance_xp2_xpyp — ADG1 upwind semi-implicit tridiagonal advance of rtp2/thlp2/rtpthlp/up2/vp2 with pr1/pr2, hole filling and all clips — plus clip_covars_denom, goldened DIRECTLY against the real public routine; clip_covars_denom bitwise, advance <= 8.5e-14 rel; D: Lscale/tau infrastructure — compute_mixing_length parcel Lscale, brunt_vaisala all variants, calc_stability_correction, tau_zt/zm + tau_N2_zm (l_stability_correct_tau_zm=T), Kh, wp2/wp3 splat, calc_surface_varnce, and the inline advance_clubb_core segment composing them, validated BITWISE against the real advance_clubb_core khzm/khzt; Cx_fnc_Richardson and the perturbed-Lscale averaging proven DEAD in EAMv3); next slice: **E** (advance_xm_wpxp); see the CLUBB slicing plan below |
...
- **D (DONE)**: Lscale/tau infrastructure ->
  eam_jax/clubb/{mixing_length,stability,surface_varnce,lscale_tau}.py.
  FLAG FINDINGS (drv_lscale_config readback, asserted in the golden):
  l_stability_correct_tau_zm=T (ACTIVE; EAMv3's clubb_stabcorrect=F
  namelist flag only touches the UNRELATED l_stability_correct_Kh_N2_zm
  /l_diffuse_rtm_and_thlm), l_diag_Lscale_from_tau=F,
  l_use_C7_Richardson=l_use_C11_Richardson=l_use_wp3_pr3=F ->
  compute_Cx_fnc_Richardson is DEAD (Cx_fnc_Richardson=0; documented
  in stability.py, not ported), l_avg_Lscale=.false. is a
  COMPILE-TIME parameter in advance_clubb_core (+
  l_Lscale_plume_centered=F) -> the perturbed-Lscale calls/averaging
  are DEAD: compute_mixing_length is called ONCE with mu=5e-4;
  l_brunt_vaisala_freq_moist=F, l_use_thvm_in_bv_freq=F (active BV =
  (g/T0)*ddzt(thlm), T0=300), l_sat_mixrat_lookup=F, l_include_ice=F
  (compile-time; compute_rsat_parcel == sat_mixrat_liq exactly),
  l_andre_1978=F (compile-time, surface_varnce), l_tke_aniso=T.
  Tunables: c_K=0.2, taumax=3600, C_wp2_splat=0 (exact-zero splat
  tendencies), lambda0_stability_coef=0.03, up2_vp2_factor=2.0,
  lmin=lmin_coef*40m=4.0 (derived in setup_parameters, NOT in the
  params vector), Lscale_max=0.25*min(host_dx,host_dy).
  Ported: compute_mixing_length (the sequential per-level parcel
  up/down TKE-exhaustion loops with the Huebler vectorized first-step
  precalc, exact FP order incl. the ((zlmin+zt(j-1))-zt(i))
  association, quadratic-formula sub-grid remainder + relative-eps
  equal-dCAPE special case, nonlocal max/min-altitude smoothing,
  lminh surface-layer floor, sqrt(up*down), boundary copies, cap;
  jax lax.fori/while port), calc_brunt_vaisala_freq_sqd (all 3
  variants incl. moist Durran-Klemp — goldened via a module-flag
  toggle driver), calc_stability_correction, term_wp2/wp3_splat,
  calc_surface_varnce, and lscale_tau.lscale_tau_segment (the inline
  advance_clubb_core composition em->thvm->Lscale->tau_zt/zm->Kh->
  splat->surface-varnce branch->stability_correction->
  tau_N2_zm=tau_zm/corr->Cx=0, also returning wp2_zt for slice C/F).
  HARNESS: all routines are PUBLIC and driven directly
  (drv_compute_mixing_length with explicit mu for entrainment sweeps,
  drv_brunt_vaisala + drv_set_bv_flags, drv_stability_correction,
  drv_term_splat with explicit C_wp2_splat, drv_calc_surface_varnce,
  drv_lscale_config/drv_param_indices_lscale getters);
  drv_lscale_tau_segment is a documented VERBATIM TRANSCRIPTION of
  the inline segment validated END-TO-END: its Kh_zm/Kh_zt match the
  khzm/khzt diagnostics of the REAL advance_clubb_core BITWISE on all
  25 golden cases (khzm/khzt are assigned from the segment before any
  prognostic advance).  GOLDEN golden/clubb_lscale.npz: 25 segment
  cases (16 entry + 8 one-step-advanced states with real rcm, surface
  flux sign/magnitude sweeps, 1 elevated-surface else-branch case),
  40 compute_mixing_length columns (physically consistent +
  strongly-stable/unstable-with-cap/saturated-band/bitwise-uniform-
  dCAPE (equal-dCAPE branch both directions)/near-zero-TKE/mu-and-
  Lscale_max-sweep/randomized-inversion families; coverage asserted:
  202 cap pts, 21/56 lminh floor pts), 16x3 BV variant cases, 12
  splat (C=0 and C=2 with the five/dt clip engaged), 44
  surface-varnce sweeps (floors + both splat-guard arms, min_wp2
  guard 12/44).  MEASURED: bitwise (eager dispatch; jax.jit fusion
  adds <=4.5e-16 1-ulp noise) — surface_varnce all 44, BV dry_t0 +
  moist, splat, seg em; 1-ulp (<=4e-16 rel) thvm/sqrt_em_zt/BV
  dry_thvm/seg surface moments; Lscale family <=1.0e-12 rel direct,
  <=2.8e-12 through the segment (83-96% bitwise; container-vs-XLA
  libm exp() in exp(-mu*dzm) through the parcel recursion — the
  exhaustion stopping point is continuous so no branch amplification);
  asserted rtol=2e-11/atol=0 (no loosened fields).  Tier-0: Lscale
  in (0, Lscale_max] + cap/floor/geometric-mean identities, tau in
  (0, taumax] + tau_N2=tau_zm/corr exact + corr in [1,4], BV sign ==
  stratification sign (exact 0 for uniform thlm), surface variance
  floors/realizability + zero-flux exact floors, Lscale_up
  monotonicity in stability and TKE (the geometric mean is NOT
  monotone — down-smoothing — documented), splat sign/clip bounds,
  neutral stability_correction == 1 (tests/test_clubb_lscale.py,
  20 tests).  Note c_K10/c_K10h = 0.35 (EAMv3 overrides recorded in
  slice C) are NOT consumed by slice D after all — Kh_zm/Kh_zt use
  c_K; c_K10/c_K10h scale Km_zm/Kmh_zm inside advance_windm_edsclrm's
  caller block (slice G).
- **E (PARTIAL — WIP, session ended mid-port)**: advance_xm_wpxp: band
  solver (dgbsv/dgbsvx port — the *svx condition-estimate path needs
  dgbtrf/dgbcon/dgbtrs, check which l_use_* flag EAM hits), C6/C7
  skewness functions, semi-implicit matrix, monotonic flux limiter
  (mono_flux_limiter.F90, tridiag).
  STATE AT HANDOFF: the Fortran side is DONE and trusted — the harness
  extensions (drv entry points in drivers/clubb_driver.F90) and the
  golden archive golden/clubb_xm_wpxp.npz were generated by the real
  Fortran in the container. The JAX files
  eam_jax/clubb/{band_solver,mono_flux_limiter,advance_xm_wpxp}.py are
  IN-PROGRESS AND UNVALIDATED (no tests/test_clubb_xm_wpxp.py exists;
  the porting agent was stopped while converting the mono-flux
  limiter's sequential sweep to lax.scan). Resume by finishing the
  limiter + advance port against the existing golden; treat every
  in-progress JAX file as untrusted until replay passes at the slice-C
  standard (rtol 1e-12, atol=0).
