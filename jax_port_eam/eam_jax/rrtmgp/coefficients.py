...
COPIED UNCHANGED from ../jax_port/scream_jax/rrtmgp/coefficients.py and
re-validated against the EAM FORTRAN sources
(components/eam/src/physics/rrtmgp/f90/mo_load_coefficients.F90 and
external/rrtmgp/mo_gas_optics_rrtmgp.F90 load/init_abs_coeffs/
reduce_minor_arrays/create_idx_minor/create_flavor/
create_gpoint_flavor): the C++ the SCREAM port transcribed is a direct
port of this Fortran, and the netCDF variable names and index
conventions are identical (the "reversed dims" of the C++ views equal
the Fortran natural dimension order). Everything here is 0-based; gpt
