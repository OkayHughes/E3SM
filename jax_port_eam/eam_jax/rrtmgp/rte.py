...
COPIED from ../jax_port/scream_jax/rrtmgp/rte.py and re-validated
line-by-line against the EAM FORTRAN sources
(components/eam/src/physics/rrtmgp/external/rte/kernels/
mo_rte_solver_kernels.F90: sw_two_stream, sw_source_2str, adding,
sw_solver_2stream, lw_source_noscat, lw_transport_noscat,
lw_solver_noscat + GaussQuad; external/rte/mo_rte_sw.F90 /
mo_rte_lw.F90 driver logic: albedo/emissivity band->gpt expansion,
apply_BC, flux reduction). The kernels are algorithmically identical to
the C++ the SCREAM port transcribed (same 1e-12 k floor, epsilon
denominator guard, sqrt(epsilon) tau threshold, D=1.66/w=0.5 single
quadrature angle as EAM passes n_gauss_angles=1). EAM always runs
top_at_1 (level 1 = radiation-grid top); only that path is implemented.

ADAPTED for EAM: rte_sw/rte_lw also return broadband and by-band NET
fluxes (mo_fluxes net_broadband: dn - up), which EAM's fluxes_t carries.
...
def rte_lw(optics, sources, sfc_emis_T, gpt2band, band2gpt):
    ...
    up, bnd_up = reduce_fluxes(fu, band2gpt)
    dn, bnd_dn = reduce_fluxes(fd, band2gpt)
    return {"flux_up": up, "flux_dn": dn, "flux_net": dn - up,
            "bnd_flux_up": bnd_up, "bnd_flux_dn": bnd_dn,
            "bnd_flux_net": bnd_dn - bnd_up}
